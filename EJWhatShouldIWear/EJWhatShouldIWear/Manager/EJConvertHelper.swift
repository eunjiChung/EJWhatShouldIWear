...
func convertGRID_GPS(mode: ConversionMode, latitude: Double = 0, longitude: Double = 0, gridX: Int = 0, gridY: Int = 0) -> EJCoordinate {
    ...
    ro = re * sf / pow(ro, sn)
    var rs = EJCoordinate(latitude: 0, longitude: 0, gridX: 0, gridY: 0)
    
    switch mode {
    case .TO_GPS:
        rs.gridX = Int(latitude)
        rs.gridY = Int(longitude)
    ...
    case .TO_GRID:
        ...
        theta *= sn
        rs.gridX = Int(floor(ra * sin(theta) + XO + 0.5))
        rs.gridY = Int(floor(ro - ra * cos(theta) + YO + 0.5))
    }
